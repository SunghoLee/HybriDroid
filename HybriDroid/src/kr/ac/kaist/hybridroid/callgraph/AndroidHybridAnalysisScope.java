...
public class AndroidHybridAnalysisScope extends AnalysisScope {
  ...
  /**
	 ...
	 * @throws IOException
	 */
public static AndroidHybridAnalysisScope setUpAndroidHybridAnalysisScope(URI classpath, Set<URL> htmls, String exclusions, URI... androidLib)
    throws IOException {
  ...
  scope = setUpJsAnalysisScope(scope, htmls);
...
private static Map<Atom, Atom> nameConvertMap = new HashMap<Atom, Atom>();
	...
	private static AndroidHybridAnalysisScope setUpJsAnalysisScope(AndroidHybridAnalysisScope scope, Set<URL> htmls)
	    ...
	    for(URL url : htmls){
		    try {
		    	File f = WebUtil.extractScriptFromHTML(url, true).snd;
		    	scope.addToScope(scope.getJavaScriptLoader(), new SourceURLModule(f.toURI().toURL()));
		    	
		    	String jspath = f.getCanonicalPath();
		    	nameConvertMap.put(Atom.findOrCreateAsciiAtom(url.getPath()), Atom.findOrCreateAsciiAtom(jspath.substring(jspath.lastIndexOf(File.separator) + 1, jspath.length())));
		    	System.err.println("#Loaded html: " + url.getFile());
		    } catch (RuntimeException | UnimplementedError | Error e) {
		      SourceModule dummy = new SourceURLModule(url);
		      String dummypath = dummy.getName();
		      nameConvertMap.put(Atom.findOrCreateAsciiAtom(url.getPath()), Atom.findOrCreateAsciiAtom(dummypath));
		      scope.addToScope(scope.getJavaScriptLoader(), dummy);
		      System.err.println("[Warning] the html does not exist: " + url.getFile() + " => " + dummy);
		    }
	    }
